define(['d3'], function () {
    ...
    HistoryView.prototype = {
        ...
        commit: function (commit, message) {
            ...
            if (!commit.parent) {
                if (!this.currentBranch) {
                    throw new Error('No es una buena idea hacer commits mientras tengamos el HEAD separado.');
                }
        ...
        branch: function (name) {
            if (!name || name.trim() === '') {
                throw new Error('Debes darle un nombre a la rama.');
            }

            if (name === 'HEAD') {
                throw new Error('No puedes llamar a una rama "HEAD".');
            }

            if (name.indexOf(' ') > -1) {
                throw new Error('No puedes tener espacios en el nombre de la rama.');
            }

            if (this.branches.indexOf(name) > -1) {
                throw new Error('Rama "' + name + '" ya existe.');
            }
        ...
        deleteBranch: function (name) {
            ...
            if (name === this.currentBranch) {
                throw new Error('No se puede eliminar la rama actual.');
            }

            branchIndex = this.branches.indexOf(name);

            if (branchIndex === -1) {
                throw new Error('La rama no existe.');
            }
        ...
        checkout: function (ref) {
            ...
            if (!commit) {
                throw new Error('No se puede encontrar el commit: ' + ref);
            }
        ...
        reset: function (ref) {
            ...
            if (!commit) {
                throw new Error('No se puede encontrar el ref: ' + ref);
            }
        ...
        merge: function (ref, noFF) {
            ...
            if (currentCommit.id === mergeTarget.id) {
                throw new Error('Ya estas al dia');
        ...
        rebase: function (ref) {
            ...
            if (currentCommit.id === rebaseTarget.id) {
                throw new Error('Ya estás al día');
